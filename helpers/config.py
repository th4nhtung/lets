from decouple import Config as dConfig, AutoConfig, RepositoryIni
...
class Config:
	def __init__(self):
		try:
			config = dConfig(RepositoryIni("config.ini"))
		...
		with open('config.ini', 'w') as configfile:
			...
			parser['settings'] = { k : '' if v is None else str(v) for k, v in self._config.items() }
